    ...
    if request.method == 'POST':
        if is_ajax(request=request):
            ...
            sale_attributes = {
                ...
                #"phone": Customer.objects.get(id=int(data['phone'])),
...
@login_required(login_url="/accounts/login")
def SalesDeleteView(request, sale_id):
    """
    Args:
    ...
    try:
        ...
        sale= Sale.objects.get(id=sale_id)
        sale.delete()
        messages.success(request,'sale_id:' +sale.id + 'deleted!', extra_tags="sucess")
        return redirect('sale:sale_list')

    except Exception as e:
        messages.success(
            request, str(e)+'There was an error during the elimination!', extra_tags="danger")
        print(e)
        return redirect('sales:sales_list')
